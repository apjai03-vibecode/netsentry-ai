...
export default function Sidebar({ activeTab, setActiveTab, onOpenAuth, findingsCount = 0 }) {
  const { user, logout } = useAuth();

  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
    { id: 'audit', label: 'Active Audit', icon: FileSearch, count: findingsCount > 0 ? findingsCount : null, countColor: 'bg-rose-100 text-rose-700' },
    { id: 'captures', label: 'Capture Ingest', icon: UploadCloud },
    { id: 'findings', label: 'Findings Catalog', icon: ShieldAlert },
  ...
  return (
    <aside className="w-64 bg-white border-r border-[#E2E8F0] flex flex-col shrink-0 h-screen sticky top-0 select-none z-30">
      
      {/* Brand & Logo Header */}
      <div className="h-14 px-5 flex items-center gap-2.5 border-b border-[#E2E8F0]">
      ...
      <div className="flex-1 py-3 px-3 space-y-0.5 overflow-y-auto">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id === 'dashboard' ? 'audit' : item.id)}
